import { CancellationToken, CancellationTokenSource, QuickPickItem, window, l10n } from 'vscode';
import { Environment } from './types';

type SearchProvider<T> = (
    value: string,
    env: Environment,
    token: CancellationToken,
) => Promise<(QuickPickItem & { item: T })[]>;

export async function searchPackageWithProvider<T>(
    searchProvider: SearchProvider<T>,
    env: Environment,
): Promise<T | undefined> {
    const quickPick = window.createQuickPick();
    quickPick.placeholder = l10n.t('Enter package name to search');
